...
describe('builder theme', () => {
  ...
  test('the toggle goes from System to the opposite of the system theme, then to its own', () => {
    const cycle = (system) => {
      const seen = ['system'];
      for (let i = 0; i < 3; i += 1) {
        seen.push(nextTheme(seen.at(-1), system));
      }

      return seen;
    };

    expect(cycle('light')).toEqual(['system', 'dark', 'light', 'system']);
    expect(cycle('dark')).toEqual(['system', 'light', 'dark', 'system']);
    // Without the system theme, it is taken as light.
    expect(nextTheme('system')).toBe('dark');
    expect(nextTheme('bogus', 'dark')).toBe('system');
