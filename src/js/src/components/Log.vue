<template>
  <div class="content">
    <b-field grouped position="is-right" style="margin: 12px 0px;">
      ...
      <b-field>
        <b-dropdown ref="dateDropdown" v-model="dateFilter" @active-change="dateDropdownChange" :close-on-click="false" position="is-bottom-left">
          <template #trigger>
            <b-button style="min-width: 200px;" type="is-light" icon-right="caret-down">
              {{ dateDropdownLabel }}
            </b-button>
          </template>
          <b-dropdown-item v-for="(_, t) in dateModes" :value="t" aria-role="listitem" @click="dateDropdownClick">
            {{ t }}
          </b-dropdown-item>
          <b-dropdown-item v-if="dateModes[dateFilter] === null" separator></b-dropdown-item>
          <b-dropdown-item v-if="dateModes[dateFilter] === null" aria-role="menu-item" :focusable="false" custom>
            <b-field label="Start Time" label-position="on-border" style="width: 100%;">
              <b-datetimepicker v-model="startDate" :max-datetime="endDate">
                <template #right>
                  <b-button label="Now" type="is-primary" @click="startDate = new Date()" />
                </template>
              </b-datetimepicker>
            </b-field>
            <b-field class="py-2">
            ...
            <b-field label="End Time" label-position="on-border" style="width: 100%;">
              <b-datetimepicker v-model="endDate" :min-datetime="startDate" :disabled="endNow">
                <template #right>
                  <b-button label="Now" type="is-primary" @click="endDate = new Date()" />
                </template>
              </b-datetimepicker>
            </b-field>
          </b-dropdown-item>
      ...
      <b-field>
        <b-dropdown v-model="levelFilter">
          <template #trigger>
            <b-button style="width: 120px;" type="is-light" icon-right="caret-down">
              {{ levelFilter == "ERROR" ? "ERROR" : (levelFilter + "+") }}
            </b-button>
          </template>
          <b-dropdown-item v-for="t in knownLevels" :value="t" aria-role="listitem">{{ t }}</b-dropdown-item>
        </b-dropdown>
      </b-field>
      <b-field>
        <b-dropdown v-model="typeFilter" multiple>
          <template #trigger>
            <b-button style="width: 120px;" type="is-light" icon-right="caret-down">
              {{ typeFilter.length == 0 ? "All Types" : (typeFilter.length == 1 ? typeFilter[0] : (typeFilter[0] + " +"
                + (typeFilter.length - 1))) }}
            </b-button>
          </template>
          <b-dropdown-item v-for="t in knownTypes" :value="t" aria-role="listitem">{{ t }}</b-dropdown-item>
        </b-dropdown>
      </b-field>
      <b-field>
        <b-input style="width: 360px;" placeholder="Search log messages" v-model="searchFilter"
          icon-right="times-circle" icon-right-clickable @icon-right-click="searchFilter = ''">
        </b-input>
      </b-field>
    </b-field>
          ...
          <div class="columns row">
            <div class="log-column level-column">
              <b-tooltip :label="item.level" type="is-light" :position="index < 2 ? 'is-right' : 'is-top'">
                <b-icon :icon="getIconForLevel(item.level)[0]" size="is-small" :type="getIconForLevel(item.level)[1]"
            ...
            <div class="log-column msg-column column is-rest">
              ...
              <b-tooltip :triggers="['click']" :label="item.msg" type="is-light" multilined>
                <span class="truncate">
                  {{ item.msg }}
                </span>
              </b-tooltip>
...
const DATE_MODES = { // date dropdown options. text => seconds to go back
...
const KNOWN_TYPES = [ // keep in sync with plog/package.go
  "SECURITY",
  "SOH",
  "SCORCH",
  "PHENIX-APP",
...
export default {
  ...
  computed: {
    filteredLogs: function () {
      ...
      console.log(`${new Date().toISOString()} start filter len=${logs.length}`)
      logs = logs.filter(log => {
        if (this.knownLevels.indexOf(log.level) < currentLevel) {
          return false;
        }
        if (this.searchFilter !== "" && !log.msg.toLowerCase().includes(this.searchFilter.toLowerCase())) {
          return false;
        }
        if (this.typeFilter.length > 0 && !this.typeFilter.includes(log.type)) {
          return false;
        }

        return true;
      })
      console.log(`${new Date().toISOString()} finish filter len=${logs.length}`)
      return logs;
    },
    dateDropdownLabel: function() {
      if (this.dateModes[this.dateFilter] === null) {
        let dateOpts = {month: 'short', day: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true}
  ...
  methods: {
    ...
    dateDropdownClick() {
      if (this.dateModes[this.dateFilter] === null) { // custom range
        return;
      }
