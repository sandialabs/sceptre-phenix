...
export default {
  ...
  methods: {
    resetData() {
      ...
      this.closeRebaseCommitModals()
    },
    closeRebaseCommitModals() {
      this.rebaseModal = {
        active: false,
        unsafe: false,
        isWaiting: false,
    ...
    resizeDisk(path) {
      this.$buefy.dialog.prompt({
        message: 'Are you sure you want to resize this disk? The size must end with one of "K,M,G,T,P,E" and may be relative by prefixing with +/- (e.g., "50G" or "-512M").<br>Resizing must be accompanied by VM OS changes to either expand partitions after resizing or shrink partitions beforehand. <b class="has-text-danger">Data loss will occur if size is reduced without modifying the OS first.</b>',
        inputAttrs: {
          type: "text",
