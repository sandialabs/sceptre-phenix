    ...
    <div class="modal-card">
      ...
      <footer class="modal-card-foot">
        ...
        <b-field
          v-show="
            fileServerEnabled &&
            roleAllowed('vms/mount', 'patch', targetExp + '/' + targetVm) &&
            roleAllowed('experiments/files', 'get', targetExp)
          "
          class="ml-3"
          style="margin-bottom: 0">
          <b-select
            v-model="selectedExperimentFile"
            :disabled="
              copyingExperimentFile ||
              currentUploadProgress !== null ||
              experimentFiles.length === 0
            "
            placeholder="Copy experiment file"
            @update:modelValue="copyExperimentFile"
            expanded>
            <option
        ...
        <b-progress
          class="progress mx-3"
          v-show="copyingExperimentFile"
          indeterminate>
          Copying {{ currentCopyFileName }}
        </b-progress>
      </footer>
    </div>
  </form>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      fileServerEnabled() {
        return usePhenixStore().features.includes('file-server');
      },
    },
    ...
    methods: {
      ...
      getExperimentFiles() {
        axiosInstance
          .get(`experiments/${this.targetExp}/files`)
          .then((resp) => {
            this.experimentFiles =
              resp.data.files === null ? [] : resp.data.files;
          })
          .catch((err) => {
            this.errorDialog(`Error getting experiment files: ${err}`);
          });
      },
      ...
      copyExperimentFile(path) {
        ...
        axiosInstance
          .post(
            `experiments/${this.targetExp}/vms/${this.targetVm}/files/copy`,
            null,
            {
          ...
          .catch((err) => {
            this.errorDialog(`Error copying experiment file: ${err}`);
            this.selectedExperimentFile = null;
            this.copyingExperimentFile = false;
          });
      },
