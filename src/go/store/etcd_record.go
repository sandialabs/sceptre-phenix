...
func (e Etcd) ListRecords(namespace, prefix string) (Records, error) {
	...
	// etcd orders by escaped key, and escaping does not preserve order.
	slices.SortFunc(records, func(a, b Record) int { return strings.Compare(a.Key, b.Key) })
...
func (e Etcd) ListRecordKeys(namespace, prefix string) ([]string, error) {
	...
	for {
		...
		if len(resp.Kvs) < etcdRecordKeyPageSize {
			// Pages follow etcd's escaped key order, which escaping does not
			// preserve.
			slices.Sort(keys)
