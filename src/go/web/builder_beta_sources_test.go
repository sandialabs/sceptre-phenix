...
// TestBuilderBetaGenerateUploadRequiresCreate asserts an uploaded config needs
...
func TestBuilderBetaGenerateUploadRequiresCreate(t *testing.T) { //nolint:paralleltest // mutates package options and the environment
	...
	var (
		...
		viewer = builderBetaRole(builderBetaPolicy(
			[]string{"*", "*/*"},
			[]string{"*", "*/*"},
			[]string{"list", "get"},
		))
		creator = builderBetaRole(builderBetaPolicy(
			[]string{"configs"},
			[]string{"*", "*/*"},
...
// TestBuilderBetaGenerateResolvesIncludes asserts included topologies are
...
func TestBuilderBetaGenerateResolvesIncludes(t *testing.T) { //nolint:paralleltest // mutates package options
	...
	root := builderBetaConfig(t, "Topology", "root")
	root.Spec = map[string]any{
		"nodes":             []any{node("root-host")},
		"includeTopologies": []any{"visible", "secret", "../outside.yml"},
	}

	visible := builderBetaConfig(t, "Topology", "visible")
	visible.Spec = map[string]any{"nodes": []any{node("visible-host")}}

	secret := builderBetaConfig(t, "Topology", "secret")
	secret.Spec = map[string]any{"nodes": []any{node("secret-host")}}

	harness := newBuilderBetaHarness(t, root, visible, secret)

	role := builderBetaRole(
		builderBetaPolicy([]string{"configs"}, []string{"Topology/root", "Topology/visible"}, []string{"list", "get"}),
		builderBetaPolicy([]string{"topologies"}, []string{"root", "visible", "secret"}, []string{"list"}),
	)
