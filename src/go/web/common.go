...
var (
	// Track context cancelers and wait groups for asynchronous experiment work.
	cancelers    = make(map[string][]context.CancelFunc) //nolint:gochecknoglobals // global state
	waiters      = make(map[string]*sync.WaitGroup)      //nolint:gochecknoglobals // periodic app state
	startWaiters = make(map[string]*sync.WaitGroup)      //nolint:gochecknoglobals // startup state
...
func stopExperiment(name string) ([]byte, error) {
	...
	hadStartWorker := cancelAndWaitExperimentWorkers(name)

	if !hadStartWorker || experiment.Running(name) {
...
func cancelAndWaitExperimentWorkers(name string) bool {
	commonMu.Lock()
	cancels := cancelers[name]
	periodicWG := waiters[name]
	startWG := startWaiters[name]
	delete(cancelers, name)
	delete(waiters, name)
	delete(startWaiters, name)
	commonMu.Unlock()
