...
func (h *builderBetaHarness) decodeTopology( //nolint:ireturn // stands in for types.DecodeTopologyFromConfig
	config store.Config,
	...
	for _, name := range spec.IncludeTopologiesF {
		included, err := h.getConfig(builderBetaKindTopology + "/" + name)
		if err != nil {
...
func TestBuilderBetaFeatureGate(t *testing.T) { //nolint:paralleltest // mutates package options
	...
	// Registered after the Builder Beta routes, exactly as [Start] does, so
	// /schemas/builder/v1 falls through to the generic schema handler.
...
func TestBuilderBetaSchemaRoutePrecedence(t *testing.T) { //nolint:paralleltest // mutates package options
	...
	for _, path := range []string{"/schemas/nope/v1", "/schemas/topology/v9"} {
		recorder := harness.do(builderBetaRequest{method: http.MethodGet, path: path, user: builderBetaTestOwner})
...
// TestBuilderBetaRoutesDocumented asserts every Builder Beta route and method
// is described in the OpenAPI document the server publishes under /docs/.
func TestBuilderBetaRoutesDocumented(t *testing.T) { //nolint:paralleltest // mutates package options
	harness := newBuilderBetaHarness(t)
...
func TestBuilderBetaUnreadableDraft(t *testing.T) { //nolint:paralleltest // mutates package options
	...
	bad := harness.createDraft(builderBetaTestOwner, "bad")

	// Written by a newer phenix, at a revision its owner never read.
	harness.store.mu.Lock()
	record := harness.store.records[bapi.NamespaceDrafts][bad.ID]
	record.Value = append(bytes.TrimSuffix(record.Value, []byte("}")), `,"future":true}`...)
	harness.store.revision++
	record.Revision = harness.store.revision
	harness.store.records[bapi.NamespaceDrafts][bad.ID] = record
	harness.store.mu.Unlock()

	recorder := harness.do(builderBetaRequest{method: http.MethodGet, path: "/builder/drafts", user: builderBetaTestOwner})
	...
	recorder = harness.do(builderBetaRequest{
		method: http.MethodDelete, path: path, user: builderBetaTestPeer, role: &owner, ifMatch: bad.ETag,
	})
