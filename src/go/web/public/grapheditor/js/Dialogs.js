...
var EditDiagramDialog = function(editorUi)
{
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
        ...
        // Stored XML carries image= styles without their directory, so restore
        // them before the model is parsed or the cells lose their icons.
        var data = window.phenixRestoreStencilPaths(Graph.zapGremlins(mxUtils.trim(textarea.value)));
        var error = null;
...
function checkValue(graph, cell, ui) {
    ...
    var nodeType = 'VirtualMachine';

    try {
        if (cell.getStyle().includes("switch"))   {device = "switch";}
        if (cell.getStyle().includes("router"))   {device = "router";   nodeType = "Router";}
    ...
    if (value.hasAttribute('schemaVars')) {
        ...
        if (cell.isVertex() && device != 'switch') {
            ...
            if (type === 'kvm') {
                ...
                // The kvm schema serves every node type, so its drive default
                // covers virtual machines. Routers get their own variable.
                if (nodeType === 'Router') {
                    if (!Array.isArray(schemaVars.hardware.drives) ||
                        schemaVars.hardware.drives.length === 0) {
                        schemaVars.hardware.drives = [{}];
                    }

                    schemaVars.hardware.drives[0].image = '$DEFAULT_ROUTER_IMAGE';
                }
            }
...
function lookforvlan(graph, cell, toSwitch=null){
    ...
    try {
        ...
        for (var i = 0; i < schemaVars.network.interfaces.length; i++){
            var iface = schemaVars.network.interfaces[i];
            ...
            if (vlans.indexOf(iface.vlan) < 0 && iface.vlan != '' && iface.vlan != toSwitch) {
                disconnectedVlans.push({idx: i, vlan: iface.vlan});
                if (!vlans_in_use.hasOwnProperty(iface.vlan)){
                    vlans_in_use[iface.vlan] = true;
                }
            }

            // HACK to ensure network interfaces not manually configured after
...
var EditDataDialog = function(ui, cell)
{
    ...
    var createEditor = function() {
        ...
        // Leave room for the absolutely positioned button row below, or the
        // editor renders underneath it and looks clipped.
        editorContainer.style.height = 'calc(100% - 96px)';
        ...
        function disableFields() {
            ...
            } else if (cell.isEdge() && cell.hasAttribute('schemaVars')) {
        ...
        ui.showDialog(this.container, 480, 560, true, false, null, false); 
...
var viewJSONDialog = function(ui)
{
    ...
    edges.forEach(cell => {
        ...
        if (cell.hasAttribute('schemaVars')) {
            ...
            if (!(vlan.name in vlanAliases) && vlan.id != 0) {
                vlanAliases[vlan.name] = vlan.id;
            }
        }
    });
    ...
    let json = {};
    json.apiVersion = "phenix.sandia.gov/v1";
    json.kind = "Topology";
    json.metadata = {};
    ...
    var createEditor = function() {
        ...
        header.textContent = "Save to phēnix";
        ...
        nameLabel.setAttribute('for', 'topo-name');
        nameLabel.textContent = 'Topology Name: ';
        var nameInput = document.createElement('input');
        nameInput.setAttribute('type', 'text');
        nameInput.setAttribute('id', 'topo-name');
        nameInput.value = json.metadata.name;

        var nrow = table.insertRow(-1);
        nrow.insertCell(-1).appendChild(nameLabel);
        nrow.insertCell(-1).appendChild(nameInput);

        var scenarioLabel = document.createElement('label');
        scenarioLabel.setAttribute('for', 'scenario-name');
        scenarioLabel.textContent = 'Scenario: ';
        var scenarioSelect = document.createElement('select');
        scenarioSelect.setAttribute('name', 'scenario');
        scenarioSelect.setAttribute('id', 'scenario-name');

        var option = document.createElement('option');

        option.setAttribute('value', '');
        option.innerHTML = 'select scenario...';

        scenarioSelect.appendChild(option);

        var srow = table.insertRow(-1);
        srow.insertCell(-1).appendChild(scenarioLabel);
        srow.insertCell(-1).appendChild(scenarioSelect);
            ...
            $.ajax({
                ...
                success: function (data) {
                    for ( var i = 0; i < data.configs.length; i++ ) {
                        let scenario = data.configs[i].metadata.name;
                        var option = document.createElement('option');
        ...
        var dlBtn = mxUtils.button(mxResources.get('apply'), function() {
            try {
                ...
                if (scenarioSelect.value) { // creating topology and experiment
                    let payload = {
                    ...
                    $.ajax({
                        ...
                        headers,
                        success: function () {
                            newDiv.html('The ' + payload.name + ' topology and experiment were created in the phēnix store');
                        ...
                        error: function (e) {
                            ...
                            if ( err && err.metadata && err.metadata.type ) {
                                ...
                                if ( err.metadata.type == "experiment" ) {
                                    msg.push('However, the topology <strong>was</strong> successfully added to the phēnix store.')
                                }
                            ...
                            newDiv.dialog({title: 'Error'}).parent().addClass('ui-state-error');
                            ui.hideDialog.apply(ui, arguments);
                        }
                    });
        ...
        dlBtn.innerHTML = 'Add Topology';
        dlBtn.setAttribute('id', 'dowloadJSON');

        scenarioSelect.addEventListener('change', function() {
            if (this.value) {
                header.innerText = 'Create Experiment in phēnix Store'
                dlBtn.innerHTML = 'Create Experiment';
            } else {
                header.innerText = 'Add Topology to phēnix Store'
                dlBtn.innerHTML = 'Add Topology';
            }
        });
        ...
        // show dialog only after editor is created
        ui.showDialog(this.container, 480, 620, true, false, null, false); 
        progressDestroy();
...
var MiniResponseDialog = function(editorUi)
{
    var div = document.createElement('div');
    div.style.overflow = 'auto';
    // Leave room for the absolutely positioned button row below, or the last
    // rows of the response scroll underneath it.
...
window.experiment_vars = [
    ...
    {name: "DEFAULT_VM_IMAGE", value:"ubuntu.qc2"},
    {name: "DEFAULT_ROUTER_IMAGE", value:"vyos.qc2"},
];
