...
Actions.prototype.init = function()
{
    ...
    this.addAction('open...', function()
    {
        ...
        header.textContent = "Import from phēnix";
        header.style.marginTop = "0";
        header.style.marginBottom = "10px";
        header.style.textAlign = 'left';

        var topoSelect = document.createElement('select');
        topoSelect.setAttribute('name', 'topology');
        topoSelect.setAttribute('id', 'topology-name');

        var option = document.createElement('option');

        option.setAttribute('value', '');
        option.innerHTML = 'select topology...';
            ...
            $.ajax({
                ...
                success: function (data) {
                    for ( var i = 0; i < data.topologies.length; i++ ) {
                        let topology = data.topologies[i];
                        var option = document.createElement('option');

                        option.setAttribute('value', topology);
                        option.innerHTML = topology;

                        topoSelect.appendChild(option);
                    }
                },
                error: function (e) {
                    console.log('error getting builder topologies');
                    mxUtils.alert(e);
                }
            });
        } catch (e) {
            ...
            try {
                ...
                if (topoSelect.value) {
                    $.ajax({
                        ...
                        headers,
                        success: function (data) {
                            // Saving strips an image's directory; put it back.
                            var cleanData = window.phenixRestoreStencilPaths(data);
                        ...
                        error: function (e) {
                            ...
                            var msg = [
                                'There was an error getting the topology from the phēnix store:',
                    ...
                    mxUtils.alert("no topology selected by user");
                }
            } catch (e) {
                console.log('catch');
                mxUtils.alert(e);
        ...
        dlBtn.className = 'geBtn';
        dlBtn.innerHTML = 'Open';

        buttons.appendChild(cancelBtn);
        buttons.appendChild(dlBtn);

        div.appendChild(header);
        div.appendChild(topoSelect);
    ...
    this.addAction('save...', function() { ui.saveFile(false); }).isEnabled = isGraphEnabled;
    this.addAction('saveAs...', function() { ui.saveFile(true); }).isEnabled = isGraphEnabled;
    this.addAction('export...', function() { ui.showDialog(new ExportDialog(ui).container, 300, 296, true, true); });
    this.addAction('editDiagram...', function()
    {
        var dlg = new EditDiagramDialog(ui);
        ui.showDialog(dlg.container, 620, 540, true, false);
        dlg.init();
    });
    ...
    this.addAction('generateConfig', function() 
    {
        ...
        setTimeout(function() {
            ...
            ui.showDialog(dlg.container, 620, 560, true, false);
            dlg.init();
            progressDestroy();
        }, 300);
    });
    ...
    this.addAction('importJSON', function()
    {
        ...
        ui.showDialog(dlg.container, 480, 560, true, false, null, false);
        dlg.init();
    });
    ...
    {
        graph.openLink('https://phenix.sceptre.dev/latest/configuration/#builder');
    });
    ...
    this.put('about', new Action(mxResources.get('about') + ' Graph Editor...', function()
    {
        if (!showingAbout)
        {
            // AboutDialog supplies its own Close button, so the corner image is
            // redundant. Stay closable so clicking the backdrop still dismisses,
            // and pass hideCloseImage to drop the image only.
            ui.showDialog(new AboutDialog(ui).container, 320, 360, true, true, function()
            {
                showingAbout = false;
