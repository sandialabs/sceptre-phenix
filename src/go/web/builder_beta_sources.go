...
func (b *builderBetaAPI) generateDocument(w http.ResponseWriter, r *http.Request) error {
	...
	actor, ok := builderBetaRequestActor(r)
	if !ok {
		return builderBetaForbidden(actor, "importing a builder document")
	}
...
func (b *builderBetaAPI) generationSource(
	...
	if !builderBetaBaseAllowed(actor.role, builderBetaVerbCreate) {
		return nil, builderBetaForbidden(actor, "importing a builder document from an uploaded config")
	}
...
func (b *builderBetaAPI) bindGeneratedScenario(
	actor builderBetaActor,
	...
	name := store.ConfigFullName(builderBetaKindScenario, ref.Name)
	plausible := ref.Name != "" && strings.TrimSpace(ref.Name) == ref.Name && !strings.Contains(ref.Name, "/")

	if plausible &&
		builderBetaBaseAllowed(actor.role, builderBetaVerbList, name) &&
		builderBetaKindAllowed(actor.role, builderBetaScenarios, ref.Name) {
...
func (b *builderBetaAPI) storedSource(
	...
	if !builderBetaBaseAllowed(actor.role, builderBetaVerbGet, name) {
		return nil, builderBetaForbidden(actor, "importing a builder document from "+name)
	}
...
// caller's behalf.
func (b *builderBetaAPI) includedTopologyLoader(actor builderBetaActor) bdoc.TopologyLoader {
	return func(name string) (*store.Config, error) {
		...
		full := builderBetaKindTopology + "/" + name

		if !builderBetaBaseAllowed(actor.role, builderBetaVerbGet, full) ||
			!builderBetaKindAllowed(actor.role, builderBetaTopologies, name) {
