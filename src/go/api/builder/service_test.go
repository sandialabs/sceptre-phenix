...
func TestCorruptDraftIsSkippedAndCanBeDeleted(t *testing.T) {
	h := newHarness(t)
	ctx := context.Background()

	good := createTestDraft(t, h, "good")
	bad := createTestDraft(t, h, "bad")
	chunks := chunkKeysOf(h, draftScope(bad.ID))

	// A field this version does not know fails the strict decoder, as a record
	// written by a newer phenix does after a rollback.
	...
	if _, err := h.service.GetDraft(ctx, bad.ID); !errors.Is(err, ErrCorrupt) {
		t.Fatalf("GetDraft error = %s, want ErrCorrupt", fmtErr(err))
	}

	all, err := h.service.ListDrafts(ctx)
	if err != nil {
		t.Fatalf("ListDrafts error = %s, want the readable drafts listed", fmtErr(err))
	}

	if len(all) != 1 || all[0].ID != good.ID {
		t.Fatalf("ListDrafts = %+v, want only %s", all, good.ID)
	}

	if owned, err := h.service.ListDraftsByOwner(ctx, testOwner); err != nil || len(owned) != 1 {
		t.Fatalf("ListDraftsByOwner = %d drafts, error %s, want the readable draft", len(owned), fmtErr(err))
	}

	// Orphan cleanup must not mistake the unreadable draft's chunks for
	// orphans, nor stop cleaning up everything else.
	orphan := storePayload(t, h, snapshotScope("vanished-draft", "vanished-snapshot"), []byte(randomText(9, 2000)))

	h.passOrphanGracePeriod()

	if removed, err := h.service.CleanupOrphanedChunks(ctx); err != nil || removed != len(orphan.ChunkDigests) {
		t.Fatalf("CleanupOrphanedChunks = %d, %s; want the %d orphaned chunks removed",
			removed, fmtErr(err), len(orphan.ChunkDigests))
	}

	if got := chunkKeysOf(h, draftScope(bad.ID)); len(got) != len(chunks) {
		t.Fatalf("chunks = %v, want the unreadable draft's chunks kept (%v)", got, chunks)
	}

	// Its owner can still be read, so the web layer can authorize deleting it.
	owner, err := h.service.GetDraftOwner(ctx, bad.ID)
	if err != nil {
		t.Fatalf("GetDraftOwner returned error: %v", err)
	}

	if owner.ID != bad.ID || owner.Owner != testOwner || owner.Revision != bad.Revision {
		t.Fatalf("GetDraftOwner = %+v, want %s owned by %s at revision %d", owner, bad.ID, testOwner, bad.Revision)
	}

	if err := h.service.DeleteDraft(ctx, bad.ID, testActor, owner.Revision); err != nil {
		t.Fatalf("DeleteDraft returned error: %v", err)
	}

	if got := chunkKeysOf(h, draftScope(bad.ID)); len(got) != 0 {
		t.Fatalf("chunks = %v, want the deleted draft's chunks removed", got)
	}

	// A record that does not even name an owner stays unreadable.
	h.store.setValue(NamespaceDrafts, good.ID, []byte("{not json"))

	if _, err := h.service.GetDraftOwner(ctx, good.ID); !errors.Is(err, ErrCorrupt) {
		t.Fatalf("GetDraftOwner of unparsable metadata error = %s, want ErrCorrupt", fmtErr(err))
	}
}
...
func TestAppendSnapshotAdvancesHistoryAndRecordsActor(t *testing.T) {
	...
	// The client's id for a save is kept with its snapshot, so a client that
	// never saw the response can tell it was stored; one that is not an id
	// is refused.
	request := AppendSnapshotRequest{
		DraftID: meta.ID, Actor: testActor, ExpectedRevision: updated.Revision,
		Document: testDocument(t, "topo-v3", 0), Summary: "v3", OpID: "not an id",
	}

	if _, err := h.service.AppendSnapshot(ctx, request); !errors.Is(err, ErrInvalid) {
		t.Fatalf("AppendSnapshot with opId %q error = %s, want ErrInvalid", request.OpID, fmtErr(err))
	}

	request.OpID = "4f1c2a90-8d3e-4b7a-9c61-2e5f7a8b9c0d"

	third, err := h.service.AppendSnapshot(ctx, request)
	if err != nil {
		t.Fatalf("AppendSnapshot with an opId returned error: %v", err)
	}

	if got := third.Current().OpID; got != request.OpID {
		t.Fatalf("snapshot opId = %q, want %q", got, request.OpID)
	}

	if got := third.History[1].OpID; got != "" {
		t.Fatalf("opId of a snapshot saved without one = %q, want none", got)
	}
}
...
// errAmbiguous stands in for a store error that does not say whether the write
// happened, such as an etcd request that timed out after its proposal applied.
var errAmbiguous = errors.New("etcdserver: request timed out")

func TestAmbiguousWriteErrorsKeepCommittedChunks(t *testing.T) {
	h := newHarness(t)
	ctx := context.Background()

	// The store applies each draft write, then reports it as failed.
	...
	appended, err := h.service.AppendSnapshot(ctx, AppendSnapshotRequest{
		DraftID: created.ID, Actor: testActor, ExpectedRevision: created.Revision,
		Document: testDocument(t, "topo-v2", 0), Summary: "v2",
	})
	if err != nil {
	...
	stored, err := h.service.GetDraft(ctx, created.ID)
	if err != nil {
		t.Fatalf("GetDraft returned error: %v", err)
	}

	switch {
	case len(stored.History) != 2 || stored.Cursor != 1:
		t.Fatalf("history = %d, cursor = %d, want the committed append", len(stored.History), stored.Cursor)
	case stored.Revision != appended.Revision:
		t.Fatalf("revision = %d, want the %d the append returned", stored.Revision, appended.Revision)
	}

	// Both snapshots are still readable: their chunks were not removed.
	for i := range stored.History {
		if _, err := h.service.GetSnapshot(ctx, created.ID, stored.History[i].ID); err != nil {
			t.Fatalf("GetSnapshot(%s) returned error: %v", stored.History[i].ID, err)
		}
	}

	// When another writer moved the draft on before it could be read back, the
	// applied append is reported as a conflict, not as a success at a revision
	// that includes someone else's change. The hook runs under the store lock,
	// so it edits the stored record directly.
	h.store.afterWrite = func(namespace, key string) error {
		if namespace != NamespaceDrafts {
			return nil
		}

		record := h.store.records[namespace][key]

		var moved DraftMetadata
		if err := json.Unmarshal(record.Value, &moved); err != nil {
			return err
		}

		moved.Cursor = 0

		value, err := json.Marshal(&moved)
		if err != nil {
			return err
		}

		h.store.revision++
		record.Value, record.Revision = value, h.store.revision
		h.store.records[namespace][key] = record

		return errAmbiguous
	}

	_, err = h.service.AppendSnapshot(ctx, AppendSnapshotRequest{
		DraftID: created.ID, Actor: testActor, ExpectedRevision: stored.Revision,
		Document: testDocument(t, "topo-v3", 0), Summary: "v3",
	})
	if !errors.Is(err, ErrConflict) {
		t.Fatalf("AppendSnapshot error = %s, want ErrConflict", fmtErr(err))
	}
	...
	if _, err := h.service.GetSnapshot(ctx, created.ID, stored.History[len(stored.History)-1].ID); err != nil {
		t.Fatalf("GetSnapshot of the applied append returned error: %v", err)
	}
}

func TestAmbiguousWriteErrorsLeaveUncommittedChunksToCleanup(t *testing.T) {
	h := newHarness(t)
	ctx := context.Background()

	meta := createTestDraft(t, h, "topo")
	before := chunkKeysOf(h, draftScope(meta.ID))

	// The store reports the same error without applying the write.
	h.store.failUpdate = func(namespace, _ string) error {
		if namespace != NamespaceDrafts {
			return nil
		}
	...
	if !errors.Is(err, errAmbiguous) {
		t.Fatalf("AppendSnapshot error = %s, want the store error", fmtErr(err))
	}
	...
	_, err = h.service.CreateDraft(ctx, CreateDraftRequest{
		Owner: testOwner, Actor: testActor, Title: "", SourceToken: "",
		Document: testDocument(t, "other", 0), Summary: "", ID: "other",
	})
	if !errors.Is(err, errAmbiguous) {
		t.Fatalf("CreateDraft error = %s, want the store error", fmtErr(err))
	}

	// A write that has not been applied yet may still be, so the attempts'
	// chunks are kept until orphan cleanup finds them unreferenced.
	if got := chunkKeysOf(h, draftScope(meta.ID)); len(got) != len(before)+1 {
		t.Fatalf("chunks = %v, want the append attempt's chunk kept beside %v", got, before)
	}

	h.passOrphanGracePeriod()

	if removed, err := h.service.CleanupOrphanedChunks(ctx); err != nil || removed != 2 {
		t.Fatalf("CleanupOrphanedChunks = %d, error %s, want both attempts' chunks removed", removed, fmtErr(err))
	}

	switch {
	case len(chunkKeysOf(h, draftScope(meta.ID))) != len(before):
		t.Fatal("orphan cleanup must remove the append attempt's chunks and keep the draft's")
	case len(chunkKeysOf(h, draftScope("other"))) != 0:
		t.Fatal("orphan cleanup must remove the create attempt's chunks")
	}
...
func TestLongerLegacyHistoryIsReadableAndPrunedOnAppend(t *testing.T) {
	h := newHarness(t)
	ctx := context.Background()
	...
	// Drafts written before the snapshot limit was lowered hold up to 100
	// snapshots. Stand in for the older ones with copies of the first.
	record, err := h.store.GetRecord(NamespaceDrafts, meta.ID)
	if err != nil {
		t.Fatalf("GetRecord returned error: %v", err)
	}

	var legacy DraftMetadata
	if err := json.Unmarshal(record.Value, &legacy); err != nil {
		t.Fatalf("decoding the draft record: %v", err)
	}

	older := make([]SnapshotManifest, 0, maxStoredSnapshots-MaxSnapshots)
	for i := range maxStoredSnapshots - MaxSnapshots {
		manifest := legacy.History[0].clone()
		manifest.ID = "legacy-" + strconv.Itoa(i)
		older = append(older, manifest)
	}

	legacy.History = append(older, legacy.History...)
	legacy.Cursor = len(legacy.History) - 1

	value, err := json.Marshal(&legacy)
	if err != nil {
		t.Fatalf("encoding the draft record: %v", err)
	}

	h.store.setValue(NamespaceDrafts, meta.ID, value)

	stored, err := h.service.GetDraft(ctx, meta.ID)
	if err != nil {
		t.Fatalf("GetDraft of a %d-snapshot draft returned error: %v", maxStoredSnapshots, err)
	}

	if len(stored.History) != maxStoredSnapshots {
		t.Fatalf("history length = %d, want %d", len(stored.History), maxStoredSnapshots)
	}

	// The next append prunes it to the current limit.
	meta = appendTestSnapshot(t, h, stored, "v"+strconv.Itoa(MaxSnapshots), testActor)

	switch {
	case len(meta.History) != MaxSnapshots || meta.Cursor != MaxSnapshots-1:
		t.Fatalf("history length = %d, cursor = %d, want %d snapshots", len(meta.History), meta.Cursor, MaxSnapshots)
	case meta.History[0].Summary != "v1":
		t.Fatalf("oldest retained snapshot = %q, want v1", meta.History[0].Summary)
	}
...
func TestPruneHistoryEnforcesTheByteLimit(t *testing.T) {
	meta := &DraftMetadata{
		ID:      "draft",
	...
	// 12 x 5 MiB is past the 50 MiB byte limit, so the two oldest snapshots
	// are pruned, well before the snapshot count limit is reached.
	switch {
	case len(dropped) != 2 || dropped[0].ID != "snap-0" || dropped[1].ID != "snap-1":
		t.Fatalf("dropped = %+v, want snap-0 and snap-1", dropped)
	case len(meta.History) != 10 || meta.Cursor != 9:
		t.Fatalf("history length = %d, cursor = %d, want 10 and 9", len(meta.History), meta.Cursor)
	...
	// A snapshot larger than the whole budget is kept, as the cursor snapshot
	// always is: AppendSnapshot rejects the edit that would store it.
	meta.History = append(meta.History, SnapshotManifest{ID: "huge", Size: MaxDraftHistoryBytes + 1})
	meta.Cursor = len(meta.History) - 1

	pruneHistory(meta)

	if len(meta.History) != 1 || meta.History[0].ID != "huge" {
		t.Fatalf("history = %+v, want only the cursor snapshot left", meta.History)
	}
}
...
func TestPruneHistoryNeverDropsTheCursorSnapshot(t *testing.T) {
	...
	// Past both the snapshot count and the byte limit.
	for i := range MaxSnapshots + 10 {
		meta.History = append(meta.History, SnapshotManifest{ID: "snap-" + strconv.Itoa(i), Size: 5 << 20})
	}
