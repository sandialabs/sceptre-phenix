...
type fakeStore struct {
	...
	// under the store lock, so it must not call back into the store.
	afterWrite func(namespace, key string) error
	// stamp, when set, returns the time written records are stamped with, the
	// way a real store stamps them with its wall clock. When nil, the stamp is
...
func (f *fakeStore) CreateRecord(namespace, key string, value []byte) (store.Record, error) {
	...
	return f.committed(namespace, key, record)
}
...
func (f *fakeStore) committed(namespace, key string, record store.Record) (store.Record, error) {
	if f.afterWrite != nil {
		if err := f.afterWrite(namespace, key); err != nil {
			return store.Record{}, err
		}
	}
