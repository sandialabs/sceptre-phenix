...
// network. Like [NetworkID], it is case sensitive.
func SwitchNodeID(network string) string {
	return deterministicID("switch", network)
}

// NetworkID returns the deterministic UUID of a network (VLAN) name. VLAN
// names are case sensitive in minimega, so VLANs differing only by case get
// different networks, and different IDs.
func NetworkID(name string) string {
	return deterministicID("network", name)
}
...
// produced by [ContentDigest]. The hex must be lowercase, as the schema's
// digest pattern requires: publish compares digests as strings, so an
// uppercase one could never match.
func isContentDigest(value string) bool {
	encoded, found := strings.CutPrefix(value, digestPrefix)
	if !found || len(encoded) != digestHexLength || encoded != strings.ToLower(encoded) {
		return false
	}
